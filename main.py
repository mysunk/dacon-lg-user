...
def dnn_loss(y_true, probas_pred):
...
#%%
    ...
    '''
    params = {
        'bagging_freq': 19, 'boosting': 'gbdt', 'colsample_bynode': 0.3898135486693247,
         'colsample_bytree': 0.18842555877544384, 'force_col_wise': True, 'learning_rate': 0.017650166362369914,
         'max_bin': 5, 'max_depth': -1, 'metrics': 'auc', 'min_data_in_leaf': 25,
         'min_sum_hessian_in_leaf': 0.06737426241310715, 'n_jobs': -1, 'num_leaves': 40, 'objective': 'binary',
         'random_state': 0, 'reg_alpha': 0.8971653884407311, 'reg_lambda': 0.11885254289919682,
         'subsample': 0.960014724793571, 'tree_learner': 'data', 'verbose': -1
    }
    ...
    params ={
        'bagging_temperature': 0.0002815749995043748, 'border_count': 59, 'colsample_bylevel': 0.46663684733988225,
        'custom_loss': 'AUC', 'depth': 7, 'iterations': 816, 'l2_leaf_reg': 47, 'learning_rate': 0.07949149044006878,
        'random_strength': 1326.5863036592555, 'scale_pos_weight': 0.563214416647416, 'subsample': 0.6014021044879473,
        'thread_count': 6}

    models_2, valid_probs_2 = cat_train_model(train_X, train_y, params, 5)
    auc_score = roc_auc_score(train_y, valid_probs_2)
    print(auc_score)

#%% stacking
    meta_input = np.concatenate([valid_probs_1.reshape(-1,1), valid_probs_2.reshape(-1,1)], axis=1)
    ...
    for train_idx, val_idx in k_fold.split(meta_input):
        # split train, validation set

        X = meta_input[train_idx, :]
        valid_x = meta_input[val_idx, :]
        y = train_y[train_idx]
        valid_y = train_y[val_idx]
        ...
        model.fit(X, y, epochs=30, batch_size=32, verbose=1, validation_data=(valid_x, valid_y),
                                              callbacks = [ls, es])

        # cal valid prediction
        valid_prob = model.predict(valid_x)
        valid_probs[val_idx] = valid_prob.reshape(-1)
    roc_auc_score(train_y, valid_probs)
...
#%%
    ...
    if infer == True:
        ...
        submission['problem'] = test_prob_1.reshape(-1)
