...
def processing_errcode_v2(errortype, errorcode):
    '''
...
def generate_new_errtype():
    ...
    new_errtype = processing_errcode_v2(errortype, errorcode)
...
def transform_errtype(data):
    ...
    for day in range(1, 31):
        ...
        transformed_errcode = processing_errcode_v2(data[day]['errtype'].values,
                                                  data[day]['errcode'].values.astype(str))
        try:
...
def transform_error_data():
    ...
    # save
    np.save(f'{data_save_path}train_err_code_v2.npy', train_err_code)
    ...
    # save
    np.save(f'{data_save_path}test_err_code_v2.npy', test_err_code)
...
def extract_err_manually(data, WINDOW = 3): # 임시로는 mean 등이고library 활용할 것
    ...
    err_r = np.concatenate(
        [np.min(err_list, axis=0), np.max(err_list, axis=0), np.mean(err_list, axis=0),
         np.median(err_list, axis=0), np.var(err_list, axis=0)], axis=1)
